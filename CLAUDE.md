...
**着手前に `docs/roi-checklist.md` を通す**（2026-09-09決定）。

- 理由: このプロジェクトでは市場調査（WBS 0.10）を Phase 0 の終盤にやった結果、
  「収益化は現実的でない」と分かったのが12,800円を払う判断の直前になった。順序が逆だった
- 所要1時間。完璧な分析ではなく「1時間だけ現実を見る」のが目的
