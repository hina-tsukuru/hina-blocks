...
flowchart TB
    subgraph Week["1週間のルーチン"]
        ...
        SU["日<br/>週次レビュー 30分<br/>Jiraで完了確認→来週の5〜7枚を上位に→WBS修正"]
        MT --> FS --> SU --> MT
    end
```
...
| 日 | 週次レビュー（Jiraで完了確認 → 来週やる5〜7枚をボード上位に並べる → WBSのズレ修正） |

> ※ 「スプリント」はScrumの機能で、**カンバンボード（KAN）には存在しない**。
> カンバンでは「上から順に並べて、上から取る」で運用する。詳細は `docs/wbs.md` の週次レビューの節。
