# WBS v1.20 - Freedom系アプリ開発 & 発信プロジェクト
...
- v1.19: 2.5 を着手せず達成済みとして完了（理由を記録）。2.6 の実機UIテストの前提を追記
- v1.20: 2.6 から「録画」を外した（使い道が決まっていない素材だったため）。見積もりを2h→1hに修正
...
- 2.5 [DEV] メイン画面UI（SwiftUI）（3h）→ KAN-35 ※**着手せず達成済みで完了**
  - 完了条件（許可状態の表示 / 選択件数とピッカー / シールドのON・OFFと状態表示）は、2.1〜2.3 を実装する過程ですべて満たされた
  - **機能単位で縦に切ると、横断の「UI仕上げ」工程は自然に消える**。WBS作成時は「機能を作ってから画面を組む」想定だったが、実際は各機能をUI込みで1つずつ完成させた
  - 見栄えの作り込み（配色・アイコン・レイアウト）は必要になってから着手する判断（2026-09-19）
- 2.6 [DEV] 実機での結合動作確認（1h）→ KAN-36
  - **「録画」を外した**（2026-09-20）。理由は使い道が決まっていないため:
    - `docs/character.md` の投稿の型は「作業内容+**スクショ**」で、動画を使う型がない
    - `docs/growth.md` でスクリーンショットに触れているのは App Store 用の2か所だけで、しかも「全部 Phase 5 以降。**いまやることはない**」と明記されている
    - **録画は合否を判定しないのでテストの役に立たない**。動作確認が成立するのは「操作して期待どおりか見る」ところまでで、録画はその副産物
    - 必要になった時点（記事やXに載せると決めた時点）で撮ればよい。スクショは後からいつでも撮れる
  - **Family Controls は自動テストで担保できない**ため、ここだけは人が操作して確認する:
    - シミュレータで動作しない
    - アプリの選択トークンは iOS のピッカーしか発行できず、コードから作れない
  - **実機でUIテストを回すときは端末のロックを解除しておく**。ロック状態によってエラーが変わる:
    - ロック中: `Lost pending connection to the test runner before launch.`
    - ロック解除・失敗時: `Timed out while enabling automation mode.`
  - 真因は特定できていない（UIオートメーションの有効化 / 端末の再起動 / 実行時のロック状態 のいずれか）。
    一度 `passcodeRequired: false` を根拠にロック説を否定したが、**確認したのが実行の「後」**だったため根拠として不十分だった
  - Phase 3 で CI に実機UIテストを載せる場合、「ロック解除された実機」という前提を満たせるかを別途検討する
  - 録画時の注意: **ホーム画面にはインストール済みアプリが全部映る**。匿名運用の観点でいちばん漏れやすい。
    通知バナー・Apple ID・連絡先名・Wi-Fi名の映り込みにも注意し、収録前に集中モードをオンにする
