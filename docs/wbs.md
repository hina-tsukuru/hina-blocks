...
- 1.2 [DEV] GitHubリポジトリ作成・初期設定（2h）🔄
  ...
  - 1.2.1 ✅ リポジトリ作成・main保護設定（2026-07-26）
    - PR必須 / 承認数0（1人開発のため。CLAUDE.mdの「セルフレビュー後にマージ」に対応）
    - **管理者にも適用**（オフにすると自分だけ素通りでき、保護の意味が無くなるため）
    - force push・ブランチ削除は禁止
    - 実際に直接pushを試して `GH006: Protected branch update failed` で拒否されることを確認済み
  - 1.2.2 ✅ .gitignore（Xcode・fastlane・.DS_Store対応）、README作成
  - 1.2.3 ✅ PRテンプレート作成（詰まった点は原因/対処/再発防止の3行構成・ネタ度⭐1-3）
    - 確認事項に「公開して困る情報が混ざっていないか」を追加（実名サイト混入事故の再発防止）
