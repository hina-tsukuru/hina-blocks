# WBS v1.18 - Freedom系アプリ開発 & 発信プロジェクト
...
- v1.18: 2.3（シールドの適用/解除）の実装内容と設計判断を追記
...
- 2.3 [DEV] ManagedSettings: シールド適用/解除（4h）→ KAN-34
  - 追加したもの: `ShieldPresentation`（状態→表示・操作可否）、`ShieldController`（ManagedSettingsの操作）、`ShieldSection`（UI）
  - **ON/OFF を自前で保存しない**。`ManagedSettingsStore` の内容は iOS 側に残るので、それを読み直して状態とする。別にフラグを持つと実態とズレる余地ができる
  - **ブロック中は対象が空でも必ず解除できる**設計にした。ここが崩れるとユーザーが自分で掛けたブロックから抜け出せなくなるため、単独のテストで守っている
  - 逆に**対象が空のままでは開始できない**。「掛かっているのに何も塞がない」状態を作らないため
  - ブロック中に対象を選び直したら、掛かっているブロックも追従させる。追従しないと画面の選択内容と実際に塞がれているものが 食い違う（`ContentView` の `onChange`）
  - ストアには名前を付けた（`hinablocks.main`）。将来スケジュール別に分けられるようにするため
