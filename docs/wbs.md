# WBS v1.4 - Freedom系アプリ開発 & 発信プロジェクト
...
- 0.7 [DEV] Atlassian MCP設定（Jira/ConfluenceをClaude Codeに接続）（1.5h）→ KAN-7
  ...
  - 0.7.2 2台目MacでMCPセットアップ・接続確認 ← **未着手**
...
- 0.10 [DEV] 競合・市場調査（0.5h）→ KAN-23
  ...
  - 結論③: ただし**サードパーティにも構造的な穴がある**（設定から権限をトグルでオフにされたら終わり）。原理的に「破れないブロッカー」は作れない
  - 判断への影響: 収益化は期待しない方がよい。12,800円は事業投資ではなく**学習と発信のコスト**として評価する
  - **MVP設計への示唆**: 現在の要件は「ボタンで手動オンオフ」だが、これは自分でいつでも解除できるため標準機能と同じ弱さを持つ。Phase 4 のドッグフーディングで「すぐ解除してしまう」問題が出た場合、除外していた「解除に摩擦を入れる」を再検討する材料になる
